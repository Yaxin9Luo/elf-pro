...
def run_training(config, *, force_cpu: bool = False):
    ...
    hope_metric_hook = HopeMetricHook(
        enabled=rank == 0 and os.environ.get("HOPE_TRACKING_RANK") is not None
    )
    ...
    # `use_compile` is the kill-switch — torch < 2.4 trips on the `autocast(...)`
    # context-manager objects that show up in the autograd graph when
    # `gradient_checkpointing=True`, raising
    #   NotImplementedError: argument of type: <class 'torch.amp.autocast_mode.autocast'>
    # during dynamo's fx tracing. Either toggle off via `use_compile=false`
    # in YAML or upgrade torch.
    ...
    # Cumulative tokens seen across the (whole) world for the loss/tokens curves.
    # We count *padded* tokens at max_length per global step, since that's the
    # actual compute the model paid for. `non_pad_tokens_seen` (only the loss-
    # masked tokens) is also tracked for a cleaner data-efficiency view.
    tokens_per_step = total_batch_size * config.max_length
    tokens_seen = global_step * tokens_per_step
    non_pad_tokens_seen = 0  # accumulated from per-step batch masks
            ...
            if global_step % config.log_freq == 0:
                ...
                if rank == 0:
                    ...
                    hope_metric_hook.log_scalars({
                        ...
                        "train/lr": current_lr,
                        "train/epoch": current_epoch_progress,
