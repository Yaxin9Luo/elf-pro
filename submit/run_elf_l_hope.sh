#!/usr/bin/env bash
set -euo pipefail

CONFIG_PATH="${1:-src/configs/training_configs/train_owt_ELF-L_hope_8gpu.yml}"
PROJECT_DIR="/mnt/dolphinfs/ssd_pool/docker/user/hadoop-nlp-sh02/native_mm/luoyaxin03/projects/elf-pro"
CONDA_ROOT="/usr/local/conda"
ELF_ENV="/mnt/dolphinfs/ssd_pool/docker/user/hadoop-hldy-nlp/3A/multimodal/yaxinluo/envs/audio_jmh2_clone_yaxin"
MUON_LOCK="${PROJECT_DIR}/.muon_install.lock"

source "${CONDA_ROOT}/bin/activate" "${ELF_ENV}"
cd "${PROJECT_DIR}"

export PYTHONPATH="${PROJECT_DIR}/submit/vendor:${PROJECT_DIR}/src:${PYTHONPATH:-}"
export PYTHONUNBUFFERED=1
export TOKENIZERS_PARALLELISM=false

OUTPUT_DIR="$(python3 - "${CONFIG_PATH}" <<'PY'
import sys, yaml
with open(sys.argv[1], "r") as f:
    cfg = yaml.safe_load(f)
print(cfg["output_dir"])
PY
)"
LAUNCH_LOG_DIR="${OUTPUT_DIR}/launch_logs"
mkdir -p "${LAUNCH_LOG_DIR}"
LAUNCH_LOG="${LAUNCH_LOG_DIR}/worker_${AFO_TASK_ID:-unknown}_$(hostname)_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "${LAUNCH_LOG}") 2>&1

echo "===== ELF HOPE launch $(date '+%F %T') ====="
echo "host=$(hostname)"
echo "pwd=$(pwd)"
echo "config=${CONFIG_PATH}"
echo "output_dir=${OUTPUT_DIR}"
echo "conda_env=${CONDA_PREFIX:-}"
echo "python=$(command -v python3)"
echo "AFO_TASK_ID=${AFO_TASK_ID:-}"
echo "AFO_ENV_CLUSTER_SPEC=${AFO_ENV_CLUSTER_SPEC:-}"
python3 -u - <<'PY'
import sys, os
print("probe_python_start", flush=True)
print("python_version", sys.version, flush=True)
print("python_executable", sys.executable, flush=True)
try:
    print("probe_torch_import_start", flush=True)
    import torch
    print("torch_import_ok", torch.__version__, flush=True)
    print("probe_torch_cuda_start", flush=True)
    print("torch", torch.__version__, "cuda_available", torch.cuda.is_available(), "cuda_count", torch.cuda.device_count(), flush=True)
except Exception as e:
    print("torch_import_error", repr(e), flush=True)
try:
    print("probe_transformers_import_start", flush=True)
    import transformers
    print("transformers", transformers.__version__, flush=True)
except Exception as e:
    print("transformers_import_error", repr(e), flush=True)
try:
    print("probe_datasets_import_start", flush=True)
    import datasets
    print("datasets", datasets.__version__, flush=True)
except Exception as e:
    print("datasets_import_error", repr(e), flush=True)
try:
    print("probe_muon_import_start", flush=True)
    import muon
    print("muon", getattr(muon, "__file__", "unknown"), flush=True)
except Exception as e:
    print("muon_import_error", repr(e), flush=True)
PY

ensure_muon() {
  if python3 -c "import muon" >/dev/null 2>&1; then
    return 0
  fi

  while ! mkdir "${MUON_LOCK}" 2>/dev/null; do
    sleep 5
    if python3 -c "import muon" >/dev/null 2>&1; then
      return 0
    fi
  done

  trap 'rmdir "${MUON_LOCK}" 2>/dev/null || true' EXIT
  if ! python3 -c "import muon" >/dev/null 2>&1; then
    python3 -m pip install --no-cache-dir "muon-optimizer==0.1.0"
  fi
  python3 -c "import muon"
  rmdir "${MUON_LOCK}" 2>/dev/null || true
  trap - EXIT
}

ensure_muon

echo "===== torchrun start $(date '+%F %T') ====="
eval "HOPE_TRACKING_RANK=0 python3 -m torch.distributed.run $(python3 submit/hope_run_torch_distribute.py) src/train.py --config ${CONFIG_PATH}"
